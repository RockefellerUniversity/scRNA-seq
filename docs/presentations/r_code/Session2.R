...
## -----------------------------------------------------------------------------
require(bluster)
clust.leiden <- clusterCells(sce.NeuroD1_filtered_QCed, use.dimred="PCA", 
                                BLUSPARAM=NNGraphParam(cluster.fun="leiden",cluster.args = list(resolution=0.1)))
clust.default <- clusterCells(sce.NeuroD1_filtered_QCed, use.dimred="PCA")



## -----------------------------------------------------------------------------
colLabels(sce.NeuroD1_filtered_QCed) <- clust.leiden
colData(sce.NeuroD1_filtered_QCed)$DefaultLabel <- clust.default
plotUMAP(sce.NeuroD1_filtered_QCed,colour_by="label")
plotUMAP(sce.NeuroD1_filtered_QCed,colour_by="DefaultLabel")
...
## plotTSNE(sce.NeuroD1_filtered_QCed,colour_by="All_Cell_Types")+facet_grid(~colour_by)
## 
