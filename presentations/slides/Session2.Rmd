...
clust.leiden <- clusterCells(sce.NeuroD1_filtered_QCed, use.dimred="PCA", 
                                BLUSPARAM=NNGraphParam(cluster.fun="leiden",cluster.args = list(resolution=0.15)),full=TRUE)
...
```{r}
tab <- table(Walktrap=sce.NeuroD1_filtered_QCed$All_Cell_Types, Louvain=clust.leiden$clusters)
...
chosen <- markers.sce.NeuroD1[["1"]]
ordered <- chosen[order(chosen$mean.AUC, decreasing=TRUE),]
head(ordered[,1:4]) # showing basic stats only, for brevity.
```

```{r}
plotExpression(sce.NeuroD1_filtered_QCed, features=head(rownames(ordered)), 
...
Nd1T.markers[Nd1T.markers$cluster == 9,]
```
