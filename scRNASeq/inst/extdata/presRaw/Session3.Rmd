...
## Identify non-empty droplets
  The *emptydrops* function can be used to identify goog/bad droplets. 2 keys arguments are:
...
.pull-left[
Pre-clean
```{r qc_mrgl,eval=TRUE,echo=TRUE,include=TRUE}
plotColData(sce,x="label", y="sum", colour_by="label")+ggtitle("read counts")
```
  ]
  
.pull-right[
Post-clean
```{r qc_mrgr,eval=TRUE,echo=TRUE,include=TRUE}
...
.pull-left[
Pre-clean
```{r qc_mrg2l,eval=TRUE,echo=TRUE,include=TRUE}
plotColData(sce,x="label", y="detected", colour_by="label")+ggtitle("gene counts")
```
  ]
  
.pull-right[
Post-clean
```{r qc_mrg2r,eval=TRUE,echo=TRUE,include=TRUE}
...
.pull-left[
Pre-clean
```{r qc_mrg3l,eval=TRUE,echo=TRUE,include=TRUE}
plotColData(sce,x="label", y="subsets_Mito_percent", colour_by="label")+ggtitle("mitocondrial content")
```
  ]
  
.pull-right[
Post-clean
```{r qc_mrg3r,eval=TRUE,echo=TRUE,include=TRUE}
...
```{r qc_complex2,eval=TRUE,echo=TRUE,include=TRUE}
...
```{r varExp3,eval=TRUE,echo=TRUE,include=TRUE}
plotExplanatoryVariables(vars)
```
