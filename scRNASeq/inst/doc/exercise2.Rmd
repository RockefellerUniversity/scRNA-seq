...
<p>&nbsp;</p>
These exercises are about manipulate single-cell data with Seurat. Please download the counting matrix from [BOX](https://rockefeller.box.com/s/d6dvkjljprvqyglul1zxudv0ktfvwhsg) and loading them into a Seurat object. Or you may also used the rds file *data/scSeq_CKO_sceSub.rds*. 
...
2. Please identify non-empty droplet and compare to the results with hard cut-off
- set limit as "100" (filter out dorplets with UMI counts less than 100)
...
3. Please mak data normalization and clustering
- Normalize and log transform the counts
- Modeling and identify top variable features
- Make UMAP plot
...
4. Please esitmate ambient RNA contamination, remove them, and test by using Hba-a1 gene
```{r,echo=toMessage}
...
5. Please re-clustering after ambient RNA removal
```{r,echo=toMessage}
set.seed(1000)
...
## Remove doublets
6. Please estimate doublets and evaluate the doublets were enriched in any clusters or not.Then try to remove the doublet cells/cclusters.
```{r,echo=toMessage}
...
- sccater plots for is.mito vs sum and detected vs sum
```{r,echo=toMessage}
library(scater)
...
8. Estimate variance explaination and try to figure out which factor is in the majority of variance.
```{r,echo=toMessage}
